...
class LeftFrame:
    ...
    def __init__(self, root, master, debug_print_flag=False):
                ...
                self.input_weight_slider1 = tk.Scale(self.controls_frame, variable=tk.DoubleVar(), orient=tk.HORIZONTAL,
                                                    from_=-10.0, to_=10.0, resolution=0.01, bg="#DDDDDD",
                                                    activebackground="#FF0000", highlightcolor="#00FFFF", label="Input Weight1",
                ...
                self.randomize = tk.Button(self.controls_frame, text="Randomize", fg="red", width=16,
                                       command=self.Random_Gen)
                self.randomize.grid(row=0, column=4)
                self.randomize = tk.Button(self.controls_frame, text="Train", fg="red", width=16,
                ...
                self.activation_function_dropdown = tk.OptionMenu(self.controls_frame, self.activation_function_variable,
                                                                   "Linear","Hyperbolic Tangent", "Hard Limit", command=lambda
...
#        x2_samples = np.random.multivariate_normal(mu_vec1, cov_mat1, 2)
        ...
        self.x1_samples=np.insert(self.x1_samples_old, [2], [[-1],[-1]], axis=1)
        self.x2_samples=np.insert(self.x2_samples_old, [2], [[1],[1]], axis=1)
        self.x1_samples_new=np.concatenate((self.x1_samples,self.x2_samples),axis = 0)
    ...
    def train_Data(self):
        for x in range(100):
            self.input_weight,self.input_weight1,self.bias=Raina_02_02.calculate_activation_function(self.input_weight,self.input_weight1, self.bias, self.x1_samples_new,
