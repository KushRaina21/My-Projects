...
class LeftFrame:
    ...
    def __init__(self, root, master, debug_print_flag=False):
                ...
                self.delayed_element_value = 10
                self.no_of_iterations = 10
                self.training_sample=80
                self.stride=1
                ...
                #data=self.read_csv_as_matrix('data_set_2.csv')
                data=self.read_csv_as_matrix('stock_data.csv')
                self.price=data[:,:1]
                self.volume=data[:,1:]
                vol=self.volume
                pri=self.price
                scaler = MinMaxScaler(feature_range=(-1, 1), copy=True)
                self.priceaftermean=scaler.fit_transform(self.price)
                a=self.priceaftermean
                self.volaftermean=scaler.fit_transform(self.volume)
                b=self.volaftermean
#                pricebefore=self.price
#                volbefore=self.volume
#                pricemean=np.mean(self.price, keepdims=True)
#                volumemean=np.mean(self.volume, keepdims=True)
#                pricestd=np.std(self.price)
#                volumestd=np.std(self.volume)
#                priceaftermean=self.price-pricemean
#                volaftermean=self.volume-volumemean
#                priceaftermean=np.true_divide(priceaftermean,pricestd)
#                volaftermean=np.true_divide(volaftermean,volumestd)
#                print(priceaftermean)
...
#                self.test_onehot=test_onehot[:,1:] 
                ...
                self.delayed_element_slider = tk.Scale(self.controls_frame, variable=tk.DoubleVar(), orient=tk.HORIZONTAL,
                                                    from_=0, to_=100, resolution=1, bg="#DDDDDD",
                                                    activebackground="#FF0000", highlightcolor="#00FFFF", label="Delayed Elements",
                                                    command=lambda event: self.delayed_element_slider_callback())
                self.delayed_element_slider.set(self.delayed_element_value)
                ...
                self.training_sample_slider = tk.Scale(self.controls_frame, variable=tk.DoubleVar(), orient=tk.HORIZONTAL,
                                                    from_=0, to_=100, resolution=1, bg="#DDDDDD",
                                                    activebackground="#FF0000", highlightcolor="#00FFFF", label="Training Sample",
                ...
                self.no_of_iterations_slider = tk.Scale(self.controls_frame, variable=tk.DoubleVar(), orient=tk.HORIZONTAL,
                                                    from_=10, to_=100, resolution=1, bg="#DDDDDD",
                ...
                #Creating 1st weight slider 
                self.no_of_iterations_slider.set(self.no_of_iterations)
                self.no_of_iterations_slider.bind("<ButtonRelease-1>", lambda event: self.no_of_iterations_slider_callback())
                
                self.no_of_iterations_slider.grid(row=0, column=4, sticky=tk.N + tk.E + tk.S + tk.W)                    
            
                
                
                # Randomize weight button
                self.set_weight_zero = tk.Button(self.controls_frame, text="Set Weights to Zero", fg="red", width=20,
                                       command=self.set_weight_zero_function)
                self.set_weight_zero.grid(row=0, column=5)
                self.set_weight_zero_function()
                self.adjust_lms = tk.Button(self.controls_frame, text="Adjust Weights(LMS)", fg="red", width=16,
                                       command=self.adjust_lms_function)
                self.adjust_lms.grid(row=0, column=6)
                
                self.adjust_weights_direct = tk.Button(self.controls_frame, text="Adjust Weights Direct", fg="red", width=20,
                                       command=self.adjust_weights_direct_func)
                self.adjust_weights_direct.grid(row=0, column=7)
...
#            plt.show()           
    ...
    def set_weight_zero_function(self):
            self.weights=np.zeros((1, (2*self.delayed_element_value)+2))
            self.bias=0
            #print('shape of input matrix',self.weights.shape)
    
    
    def adjust_lms_function(self):
            #print('size 1',np.size(self.priceaftermean,1))
            #print('size 0',np.size(self.priceaftermean,0))
            self.count=0
            self.itr_array=0
            self.error_array=0
            self.error_MEA_array=0
            for itr in range(0,(self.no_of_iterations)):
                self.initial=0
                self.left=np.size(self.X_train,0)
                #c=int(np.size(self.X_train,0)/(self.delayed_element_value+self.stride-1))
                while(self.left>self.stride):
                    d=self.X_train
                    f=self.left
                    
                    #final=self.delayed_element_value+x+1
    #                hj=d
    #                y=self.volaftermean[x:self.delayed_element_value+x,:]
                    end=(self.delayed_element_value+1)+(self.initial*self.stride)
                    start=end-(self.delayed_element_value+1)
                    #print('end',end)
                    #print('start',end)
                    self.dataVal=np.row_stack(((self.X_train[start:end,:]),(self.y_train[start:end,:])))
                    self.target=self.X_train[end,:]
                    self.output=np.dot(self.weights,self.dataVal)+self.bias
                    self.error=self.target-self.output
                    self.weights=self.weights+2*self.learning_rate*self.error*np.transpose(self.dataVal)
                    self.bias=self.bias+2*self.learning_rate*self.error
                    
                    dta=self.dataVal
                    #initial=initial+self.stride
                    self.left=np.size(self.X_train,0)-((self.delayed_element_value+1)+(self.initial*self.stride))
                    self.initial=self.initial+1
                    
                    f=self.left
                self.calculate_errors_test() 
                print('iteration ',itr)
                print('error ',self.errorTest)
                self.itr_array=np.row_stack((self.itr_array,itr))
                self.error_array=np.row_stack((self.error_array,self.errorTest))
                self.error_MEA_array=np.row_stack((self.error_MEA_array,self.errorMAEFinal))
                
                
                
#                self.axes.set_xlabel('Epoch')
#                self.axes.set_ylabel('Error')
#                self.axes.plot(itr,self.errorTest)
#                self.axes.xaxis.set_visible(True)
#                self.axes.set_xlim(self.xmin, self.xmax)
#                self.axes.set_ylim(self.ymin, self.ymax)
##            
#                plt.plot(itr,self.errorTest)
#                plt.axis(0, 100,0, 2)
#                #plt.ylim()
#                #plt.title(self.activation_type)
#                plt.show()
#                self.canvas.draw()                
            ...
            self.itr_array=np.transpose(self.itr_array)
            self.error_array=np.transpose(self.error_array)
            self.error_MEA_array=np.transpose(self.error_MEA_array)
            self.itr_array=self.itr_array[:,1:]
            self.error_array=self.error_array[:,1:]
            self.error_MEA_array=self.error_MEA_array[:,1:]
            #print([1,2,3,4].size)
            print(self.error_array)
            print(self.error_MEA_array)
            a=self.itr_array
            b=self.error_array
            self.axes.cla()
            self.axes.set_xlabel('Epoch')
            self.axes.set_ylabel('Error')
            self.axes.scatter(self.itr_array,self.error_array, marker='+')
            self.axes.scatter(self.itr_array,self.error_MEA_array, c= 'black', marker='o')
            #plt.plot(self.itr_array, self.error_array,'ro')
            #plt.axis([0, 10, 0, 0.1])
            self.axes.set_title("LMS Learning + for MSE 0 for MAE")
            max1=np.max(self.error_array)
            max2=np.max(self.error_MEA_array)
            self.axes.set_xlim(self.xmin, self.xmax)
...
#                self.h=np.dot(self.X_train,self.dataVal)
    ...
    def training_sample_slider_callback(self):
        self.training_sample_value = np.float(self.training_sample_slider.get())
        self.training_sample_value=1-(self.training_sample_value/100)
        print('split',self.training_sample_value)
        self.X_train,self.X_test,self.y_train,self.y_test=train_test_split(
        self.priceaftermean,self.volaftermean,test_size=self.training_sample_value,shuffle=False)
        print(" shape of P_train : ",  self.X_train.shape, self.X_train)
        print(" shape of P_test : ", self.X_test.shape, self.X_test)
        print(" shape of T_train : ", self.y_train, self.y_train)
        print(" shape of T_test : ", self.y_test, self.y_test)
